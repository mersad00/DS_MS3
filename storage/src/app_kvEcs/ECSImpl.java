...
public class ECSImpl implements ECS {
	...
	private List<ServerInfo> calculateMetaData(List<ServerInfo> serversToStart) {
		...
		for (int i = 0; i < serversToStart.size(); i++) {
			serversToStart.get(i).setFirstReplicaInfo(
					serversToStart.get((i + 1) % serversToStart.size()));
			serversToStart.get(i).setSecondReplicaInfo(
					serversToStart.get((i + 2) % serversToStart.size()));
		}
		...
		if (activeServers.size() <= 2) {
			...
			if (sendData(successor, newNode, newNode.getFromIndex(),
				...
				if (sendData(newNode, replicas.get(0), newNode.getFromIndex(),
					...
					/*
					 * 6.tell to the next two nodes (newNodes replicas) to
					 * delete their replicated data which they used to store
					 * from newNode's Masters
					 */
