...
public class ECSImpl implements ECS {
	...
	ServerConnection successorChannel = this.activeConnections.get(successor);
	...
	try {
	    ...
	    logger.error("Write lock message couldn't be sent."+e.getMessage());
	}
	...
	try {
	    ...
	    logger.error("move data message couldn't be sent."+e.getMessage());
	}
