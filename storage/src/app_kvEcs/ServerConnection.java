...
/**
 * This Class is responsible for communicating with the servers (sending ECS
 * commands, and receiving acknowledgments)
 *
 */
public class ServerConnection extends Thread {
	...
	private synchronized void tearDownConnection() throws IOException {
		// logger.info ( "tearing down the connection from " +
		// connection.getPort() + "..." );
		if (connection != null) {
			connected = false;
			input.close();
	...
	public ECSMessage receiveMessage() throws IOException {
		if (isConnected()) {
			...
			logger.info("Receive ECSMessage from " + connection.getPort()
					+ ":\t '" + msg.getActionType() + "'");
			return msg;
		} else
			return null;
	...
	public void sendMessage(ECSMessage msg) throws IOException {
		...
		logger.info("Send message to " + connection.getPort() + ":\t '"
				+ msg.getActionType() + "' to : " + server.toString());
	}
